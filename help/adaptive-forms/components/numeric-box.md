...
적응형 양식의 숫자 상자 구성 요소는 사용자가 숫자 값을 입력할 수 있는 양식 필드 유형입니다. 구성 요소는 일반적으로 숫자를 늘리거나 줄이기 위한 위쪽 및 아래쪽 화살표가 있는 텍스트 필드로 표시됩니다.
...
적응형 양식 숫자 상자 핵심 구성 요소는 Cloud Service의 핵심 구성 요소 2.0.4 및 AEM 6.5.16.0 Forms 이상의 핵심 구성 요소 1.1.12 일부로 2023년 2월에 릴리스되었습니다. 다음 테이블에서는 지원되는 모든 버전, AEM 호환성 및 해당 문서에 대한 링크를 보여 줍니다.
...
2. 드롭다운 메뉴에서 기존 정책을 `numberinput-default-policy`로 선택합니다.

   ![날짜 선택기 템플릿 정책](/help/adaptive-forms/assets/numberinput-template-policy.png)
...
디자인 대화 상자는 숫자 상자 구성 요소의 CSS 스타일을 정의하고 관리하는 데 사용됩니다.
...
- **기본 CSS 클래스**: 적응형 양식 숫자 상자 핵심 구성 요소에 기본 CSS 클래스를 제공할 수 있습니다.
...
형식 탭에서는 기본 및 사용자 정의 날짜 형식을 지정할 수 있습니다. 또한 **추가** 버튼을 사용하여 새로운 사용자 정의 숫자 형식을 추가할 수도 있습니다.
...
템플릿 정책에 형식을 추가하려면 다음 작업을 수행합니다.

1. 템플릿 편집기에서 적응형 양식 또는 Sites와 연결된 해당 템플릿을 엽니다.
2. **허용된 구성 요소** 섹션에서 **[!UICONTROL 숫자 입력]** 구성 요소를 선택합니다.
   ![날짜 선택기 템플릿 정책](/help/adaptive-forms/assets/number-input-policy.png)
3. **[!UICONTROL 추가]** 아이콘을 클릭하여 숫자 입력 구성 요소에 대한 새 정책을 만듭니다.
4. 정책 이름을 입력하고 **형식** 탭에서 필요한 형식을 모두 선택합니다.
   ![정책 만들기](/help/adaptive-forms/assets/date-picker-format-policy.png)
