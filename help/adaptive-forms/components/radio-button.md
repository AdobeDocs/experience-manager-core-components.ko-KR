---
title: 응용 Forms 코어 구성 요소 - 라디오 단추
description: 응용 Forms 라디오 단추 코어 구성 요소 사용 또는 사용자 지정
...
# 라디오 단추 {#radio-button-adaptive-forms-core-component}

적응형 양식의 라디오 단추는 사용자가 관련 옵션 그룹에서 하나의 옵션을 선택할 수 있는 입력 요소의 유형입니다. 옵션 선택 여부를 나타내기 위해 채워지거나 비어 있는 작은 원형 단추로 표시됩니다. 사용자가 하나의 라디오 단추를 선택하면 그룹의 다른 단추가 선택 취소됩니다. 라디오 단추는 일반적으로 여러 가지 상호 배타적인 옵션이 있고 한 번에 하나만 선택할 수 있는 경우 사용됩니다.
...
이 예에서 Options 요소는 라디오 단추를 함께 그룹화하는 데 사용됩니다. 다음 **텍스트 표시** 요소를 사용하여 항목 및 **데이터 값** 양식을 제출할 때 서버로 전송되는 값을 지정하는 데 사용됩니다.

각 라디오 단추 옵션에는 고유한 데이터 값과 텍스트 표시 속성이 있습니다. 사용자가 &quot;1-10&quot;을 선택하면 양식을 제출할 때 해당 데이터 값이 서버로 전송됩니다. 그런 다음 서버측 스크립트로 이 데이터를 처리하여 사용자가 선택한 옵션을 확인하고, 추가 처리를 위해 양식의 다른 필드 업데이트나 양식 데이터를 서버측 스크립트에 제출하는 등의 다양한 작업을 수행하는 데 사용할 수 있습니다.

또한 각 라디오 단추에는 각 옵션에 대해 다른 처리 값이 있도록 구성할 수 있으며, 적응형 Forms 규칙 편집기를 사용하여 설정할 수 있습니다

## 사용 {#reasons-to-use-radio-button}

다음과 같은 여러 가지 이유로 양식에 라디오 단추를 사용합니다.

* **제한된 선택**: 라디오 단추는 사용자가 선택할 수 있는 사전 정의된 옵션 목록을 제공하는 데 사용되며 한 번에 하나의 옵션만 선택할 수 있습니다. 이 기능은 옵션 수가 제한되고 상호 배타적일 때 유용합니다.

* **표시 지우기**: 라디오 단추는 명확하고 이해하기 쉬워 사용자가 어떤 것을 선택하는지 쉽게 알 수 있습니다.

* **일관성**: 라디오 단추를 사용하면 일관되고 표준화된 방식으로 사용자에게 옵션을 제공할 수 있으므로 양식을 쉽게 이해하고 상호 작용할 수 있습니다.

* **사용하기 쉬움**: 라디오 단추는 기술에 익숙하지 않거나 이동성이 제한된 사용자에게 사용하기 쉽습니다.

## 버전 및 호환성 {#version-and-compatibility}

응용 Forms 라디오 단추 코어 구성 요소는 코어 구성 요소 2.0.4의 일부로 2023년 2월에 출시되었습니다. 다음은 지원되는 모든 버전, AEM 호환성 및 해당 설명서에 대한 링크를 보여주는 표입니다.
...
기술 설명서에서 응용 Forms 라디오 단추 코어 구성 요소에 대한 최신 정보를 가져옵니다. [GitHub](https://github.com/adobe/aem-core-forms-components/tree/master/ui.af.apps/src/main/content/jcr_root/apps/core/fd/components/form/radiobutton/v1/radiobutton). 핵심 구성 요소 개발에 대한 자세한 내용은 [핵심 구성 요소 개발자 설명서](/help/developing/overview.md).

## 구성 대화 상자 {#configure-dialog}

구성 대화 상자에서 방문자에 대한 라디오 단추 경험을 쉽게 사용자 지정할 수 있습니다. 또한 원활한 사용자 경험을 위해 라디오 단추 옵션을 쉽게 정의할 수 있습니다.
...
* **제목 숨기기** - 구성 요소의 제목을 숨기려면 옵션을 선택합니다.
   ...
   에서 **옵션** 탭에서 데이터 값을 추가하고 텍스트 쌍을 표시할 수 있습니다 **추가** 버튼을 클릭합니다. 새 옵션이 추가되면 다음 작업을 수행할 수 있습니다.

   * **데이터 값** - 이 옵션을 사용하면 옵션을 선택할 때 제출할 컨텐츠를 입력할 수 있습니다.
   * **텍스트 표시** - 이 옵션을 사용하면 적응형 양식에 표시할 컨텐츠를 입력할 수 있습니다.
   * **삭제** - 라디오 단추 옵션을 삭제하려면 탭하거나 클릭합니다.
   * **다시 정렬** - 탭하거나 클릭하고 드래그하여 옵션 순서를 재정렬합니다.
...
* **기본 옵션** - 이 옵션을 사용하면 양식이 로드될 때 미리 선택한 기본값을 추가할 수 있습니다. 만약 **제출된 값의 데이터 유형** 가 로 설정되어 있습니다. `Number` 그리고 문자열 데이터를 **기본 옵션**&#x200B;로 설정되면 화면에는 `Value type mismatch` 오류 메시지.

* **표시 옵션** - 이 옵션은 적응형 양식에서 라디오 단추의 시각적 정렬을 설정하는 데 사용됩니다. 지원되는 두 옵션은 다음과 같습니다.
   * **가로** - 이 옵션을 선택하면 적응형 양식에 라디오 단추가 왼쪽으로 오른쪽으로 표시됩니다.
   * **세로** - 이 옵션을 선택하면 적응형 양식에서 라디오 단추가 맨 위에서 맨 아래로 표시됩니다.
...
![액세스 가능성 탭](/help/adaptive-forms/assets/radiobutton_accessibilitytab.png)
...
디자인 대화 상자는 라디오 단추 구성 요소의 CSS 스타일을 정의하고 관리하는 데 사용됩니다.


### 스타일 탭 {#styles-tab}

디자인 대화 상자는 구성 요소의 CSS 스타일을 정의하고 관리하는 데 사용됩니다. 응용 Forms 라디오 단추 코어 구성 요소는 AEM을 지원합니다 [스타일 시스템](/help/get-started/authoring.md#component-styling).

**기본 CSS 클래스**: 응용 Forms 라디오 단추 코어 구성 요소에 기본 CSS 클래스를 제공할 수 있습니다.
