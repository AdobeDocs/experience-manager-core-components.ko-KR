...
<span class="preview"> **제출된 값의 데이터 형식** 기능은 얼리어답터 프로그램에서 사용할 수 있습니다. 공식 이메일 ID를 사용하여 aem-forms-ea@adobe.com으로 이메일을 보내 얼리 어답터 프로그램에 참여하고 기능에 대한 액세스 권한을 요청할 수 있습니다. </span>
...
적응형 Forms 파일 첨부 핵심 구성 요소는 Cloud Service용 핵심 구성 요소 2.0.4 및 AEM 6.5.16.0 Forms 이상의 핵심 구성 요소 1.1.12의 일부로 2023년 2월에 릴리스되었습니다. 다음 표에서는 지원되는 모든 버전, AEM 호환성 및 해당 문서에 대한 링크를 보여 줍니다.
...
![기본 탭](/help/adaptive-forms/assets/fileattachement_basictab1.png)
...
- **제출된 값의 데이터 형식**: 첨부 파일을 서버에 제출하는 방법을 결정하는 옵션을 선택합니다. 첨부 파일을 이진 데이터로 보내려면 `File` 옵션을 선택합니다. 첨부 파일을 Base64로 인코딩된 문자열로 보내려면 `String` 옵션을 선택하십시오. `String`을(를) 선택하면 이진 형식의 파일이 서버에 데이터 URL로 제출됩니다. 서버는 데이터 URL을 이진 형식으로 자동 변환하여 사용자의 변경 없이 이메일 전송 및 기록 문서 생성과 같은 기존 작업과의 호환성을 보장합니다. 기본적으로 `File` 옵션이 선택되어 있습니다.
...
- **텍스트 드래그 앤 드롭** - 사용자에게 파일을 첨부하거나 드래그 앤 드롭하라는 메시지를 표시하기 위해 **첨부** 버튼의 상단에 표시되는 텍스트입니다. **첨부** 버튼의 상단에 표시되는 텍스트를 맞춤 설정할 수 있는 옵션이 있습니다. 또한 서식 있는 텍스트 메뉴를 사용하여 텍스트 서식을 지정할 수 있습니다.
