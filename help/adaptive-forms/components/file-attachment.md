...
적응형 Forms 파일 첨부 핵심 구성 요소는 2023년 2월에 Cloud Service용 핵심 구성 요소 2.0.4 및 AEM 6.5.16.0 Forms 이상의 핵심 구성 요소 1.1.12의 일부로 릴리스되었습니다. 다음 테이블에서는 지원되는 모든 버전, AEM 호환성 및 해당 문서에 대한 링크를 보여 줍니다.
...
- **제출된 값의 데이터 유형**: 첨부 파일이 서버에 제출되는 방식을 결정하려면 옵션을 선택합니다. 첨부 파일을 이진법 데이터로 보내려면 `File` 옵션을 선택합니다. 첨부 파일을 Base64 인코딩된 문자열로 보내려면 `String` 옵션을 선택합니다. `String`을 선택하면 이진법 형식의 파일이 데이터 URL로 서버에 제출됩니다. 서버는 사용자가 아무런 변경을 하지 않아도 데이터 URL을 이진법 형식으로 자동 변환하여 이메일 전송 및 기록 문서 생성과 같은 기존 작업과의 호환성을 보장합니다. 기본적으로 `File` 옵션이 선택되어 있습니다.
...
>
>
> 파일 형식을 허용된 파일 형식 형식으로 변경하여 파일을 제출하면 양식을 제출하는 동안 오류가 발생합니다.
