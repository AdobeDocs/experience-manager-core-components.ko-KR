...
적응형 Forms 핵심 구성 요소는 의미 체계 `<fieldset>` 및 `<legend>` HTML 요소를 사용하여 확인란 그룹의 액세스 가능성을 개선합니다. 이러한 요소를 사용하면 화면 판독기에서 양식 구조를 정확하게 해석하고 그룹 레이블과 해당 옵션 간의 관계를 전달할 수 있습니다.

확인란 그룹(v2) 구성 요소가 `<fieldset>` 요소 내에서 렌더링되며 그룹 레이블은 `<legend>`에 첫 번째 자식으로 래핑됩니다. 이 구조를 사용하면 화면 판독기가 개별 확인란 옵션을 읽기 전에 그룹 레이블을 발표하므로 보조 기술을 보유한 사용자가 각 옵션의 컨텍스트와 목적을 보다 쉽게 이해할 수 있습니다.
