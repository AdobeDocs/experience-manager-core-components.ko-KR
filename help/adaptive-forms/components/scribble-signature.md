...
- **A:** 캔버스에 서명을 그리려면 **그리기** 아이콘을 클릭하세요.
- **B:** 캔버스에서 서명을 지우려면 **지우기** 아이콘을 클릭하십시오.
- **C:** **지리적 위치** 아이콘을 클릭하여 서명과 함께 지리적 위치를 추가하십시오.
- **D:** 캔버스에 이름을 입력하려면 **키보드** 아이콘을 클릭하십시오.

스크리블 서명 창에서 **저장** 아이콘을 선택하면 서명을 편집할 수 없습니다. 서명을 편집하려면 현재 서명을 무시하고 위의 [페인트 브러쉬/키보드] 옵션을 사용하여 다시 서명해야 합니다.
...
다음과 같은 여러 가지 이유로 양식에 스크리블 서명 필드를 포함하는 것이 좋습니다.

- **디지털 동의**: 사용자가 법적으로 유효한 서명을 전자적으로 제공할 수 있습니다.
- **향상된 사용자 환경**: 스캔 또는 업로드 없이 장치에서 직접 서명하는 자연스러운 방법을 제공합니다.
- **페이퍼리스 워크플로**: 문서를 인쇄하고, 서명하고, 다시 검사할 필요가 없습니다.
- **인증**: 확인 및 승인의 추가 계층 역할을 합니다.
- **데이터 정확도**: 디지털 형식으로 서명자의 필기 입력을 정확하게 캡처합니다.

## 버전 및 호환성 {#version-and-compatibility}

적응형 Forms 스크리블 서명 핵심 구성 요소는 Cloud Service 이상용 **핵심 구성 요소 2.24.6**&#x200B;의 일부로 **2025년 8월**&#x200B;에 릴리스되었습니다.
...
[GitHub](https://github.com/adobe/aem-core-forms-components)의 적응형 Forms 스크리블 서명 핵심 구성 요소에 대한 최신 기술 세부 정보를 알아보십시오. 핵심 구성 요소 개발에 대한 자세한 내용은 [핵심 구성 요소 개발자 설명서](/help/developing/overview.md)를 참조하십시오.

## 구성 대화 상자 {#configure-dialog}

구성 대화 상자를 통해 스크리블 서명 구성 요소를 맞춤화할 수 있습니다.
...
- **서명 대화 상자 제목**: 서명 대화 상자 제목은 서명 캡처 대화 상자의 맨 위에 표시되는 텍스트를 정의합니다. 서명을 제공해야 할 때 사용자에게 프롬프트 또는 지침 역할을 합니다. 텍스트는 서명 프로세스를 안내하여 상호 작용을 명확하고 직관적으로 만듭니다.
...
디자인 대화 상자 를 사용하여 스크리블 서명 구성 요소의 CSS 스타일을 정의하고 관리합니다.

### 스타일 탭 {#styles-tab}

탭은 구성 요소의 CSS 스타일을 정의하고 관리하는 데 사용됩니다. 적응형 Forms 스크리블 서명 핵심 구성 요소는 AEM [스타일 시스템](/help/get-started/authoring.md#component-styling)을(를) 지원합니다.

![디자인 대화 상자](/help/adaptive-forms/assets/checkbox-style.png)

- **기본 CSS 클래스**: 적응형 Forms 스크리블 서명 핵심 구성 요소에 기본 CSS 클래스를 제공할 수 있습니다.
