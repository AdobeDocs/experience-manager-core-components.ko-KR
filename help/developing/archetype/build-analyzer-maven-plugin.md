...
다음 테이블에서는 이 단계의 일부로 실행되는 분석기를 설명합니다. <!-- Note that some are executed in the local SDK, while others are only executed during the Cloud Manager pipeline deployment. -->
