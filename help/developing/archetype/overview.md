...
* AEM 페이지 코어 [구성 요소를 기반으로 하는 페이지 구성 요소](/help/components/page.md)
* 권장 프록시 패턴으로 구현된 컨텐츠 구성 요소의 예와 AEM 코어 구성 요소를 기반으로 하는 도움말 [월드 사용자 지정 구성 요소의 예입니다](/help/introduction.md).
* 양식 구성 요소의 [예](/help/components/forms/form-container.md)
* 디바이스 에뮬레이터, 드래그 앤 드롭 설정 및 국제화 구성
* BEM 이름 지정 규칙 및 구성 요소별 스타일에 따른 클라이언트 라이브러리
* 샘플 모델, 서브렛, 필터 및 스케줄러를 포함한 예제 번들
* 단위, 통합 및 클라이언트측 테스트
* 반응 또는 각진(선택 사항)의 샘플 SPA 구현
...
* Adobe Experience Manager 6.3.3.0 이상(각도 또는 반응형 프런트 엔드 빌드 옵션을 사용하여 프로젝트를 생성할 때 6.4.2 이상) 또는 [AEM을 클라우드 서비스로 사용](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/landing/home.html)
...
| `version` |  | 버전 |
| `package` |  | Java 소스 패키지 |
| `appID` |  | 구성 요소, 구성 및 콘텐트 폴더 및 css ID에 사용되는 응용 프로그램 ID |
| `appTitle` |  | 웹 사이트 제목 및 구성 요소 그룹에 사용되는 응용 프로그램 제목 |
| `aemVersion` | 6.5.0 | Target AEM 버전 |
| `sdkVersion` |  |
| `languageCountry` | en_us | 언어 및 국가 코드를 사용하여 지역화된 컨텐츠 구조(예:( `en_us`) |
| `includeExamples` | y | 구성 요소 라이브러리 예제 사이트 포함 |
| `includeErrorHandler` | n | 사용자 지정 404 응답 페이지 포함 |
| `frontendModule` | 없음 | 전용 프런트 엔드 모듈 포함( `none`하나, [`general`](uifrontend.md), [`angular`](uifrontend-angular.md)[`react`](uifrontend-react.md)) |
| `singleCountry` | y | 예제 컨텐츠에서 언어 마스터 구조 만들기 |
| `includeDispatcherConfig` | n | Adobe Managed Services용 프로젝트를 만들 때 AEM용 프로젝트를 클라우드 서비스 <br> 세트로 만들 때 [`cloud`](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/dispatcher.cloud) 다음으로 설정된 프로젝트에 대해 [디스패처 구성이 생성되는지](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/landing/home.html) 정의합니다 <br> [`ams`](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/dispatcher.ams) . |
...
| 프로필 ID | 설명 |
--------------------------|------------------------------
| `autoInstallBundle` | felix 콘솔에 maven-sling-plugin을 사용하여 핵심 번들 설치 |
