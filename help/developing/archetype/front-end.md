...
프로젝트에 어떤 프론트엔드 개발 워크플로를 구현하든지 백엔드 개발자와 프론트엔드 개발자는 먼저 마크업에 동의해야 합니다. 일반적으로 AEM은 핵심 구성 요소에서 제공하는 마크업을 정의합니다. [그러나 필요한 경우 이를 사용자 지정할 수 있습니다.](/help/developing/customizing.md#customizing-the-markup)
...
[AEM clientlib.](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/full-stack/clientlibs.html?lang=ko)을(를) 사용하여 프론트엔드 모듈을 사용할 수 있습니다. NPM 빌드 스크립트를 실행하면 앱이 빌드되고 `aem-clientlib-generator` 패키지는 결과 빌드 출력을 가져와 clientlib으로 변환합니다.
