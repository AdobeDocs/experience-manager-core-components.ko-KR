...
* 다양한 기능
   * [다양한 사용 사례를 수용할 수 있는 유연한 구성 옵션](/help/get-started/authoring.md)
   * [페이지 작성자가 사용할 수 있는 기능을 정의하는 사전 구성 가능한 기능](/help/get-started/authoring.md#pre-configuring-core-components)
* 지속적인 전달
   * 잦은 증분 기능 개선
...
* 최신 구현
   * HTML 템플릿 [언어](https://docs.adobe.com/content/help/en/experience-manager-htl/using/overview.html) (HTL)에 정의된 마크업
...
* 액세스 가능
   * WCAG [2.0 AA 표준 준수](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/sites/authoring/fundamentals/accessible-content.html)

>[!CAUTION]
>
>핵심 구성 요소는 AEM 6.3 이상 및 Java 8이 필요하며 [편집 가능한 템플릿을 사용해야 합니다](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/sites/authoring/features/templates.html)
>
...
프로젝트에서 [핵심 구성 요소](/help/get-started/using.md) 사용을 시작하는 방법에 대한 자세한 내용은 핵심 구성 요소 사용 설명서 페이지를 참조하십시오.
...
핵심 구성 [요소의](https://adobe.com/go/aem_cmp_library)현재 릴리스를 보여주고 사용 예를 제공하는 구성 요소 라이브러리를 확인하십시오.

### 기본 구성 요소 {#out-of-the-box}

AEM을 실행하는 방법에 따라 핵심 구성 요소가 자동으로 설치되거나 설치되지 않을 수 있습니다.

#### 클라우드 서비스로 AEM 사용 {#aem-cloud-service}

핵심 구성 요소는 클라우드 서비스로 [AEM과 완전히 호환되지만](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/landing/home.html), 핵심 구성 요소는 수동으로 설치해야 하며 즉시 사용할 수 없습니다.

#### AEM 온프레미스 {#on-premise}

온-프레미스 설치에서는 핵심 구성 요소가 샘플 컨텐츠가 있으면 We.Retail 참조 사이트에서 [해당 구성 요소를](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/we-retail.html) 사용하기 때문에 빠른시작에서 표시됩니다. 그러나 프로덕션( `nosamplecontent` 런타임 모드에서 샘플 컨텐츠가 활성화되지 않은 상태로)에서 실행할 때 핵심 구성 요소가 더 이상 표시되지 않고 AEM 인스턴스에 설치되어 있어야 합니다.

>[!NOTE]
>
>온-프레미스 프로덕션 환경에서는 항상 `nosamplecontent` 실행 모드에서 Quickstart를 실행합니다. 런타임 모드에서 핵심 구성 요소를 사용하려면 핵심 구성 요소 사용 `nosamplecontent` 설명서 [](/help/get-started/using.md) 페이지의 지침을 따르십시오.
...
| 마크업 정의 | [HTML 템플릿 언어](https://docs.adobe.com/content/help/en/experience-manager-htl/using/overview.html) (HTL) 구문 | JSP 코드 |
