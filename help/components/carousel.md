...
현재 버전의 슬라이드 구성 요소는 2019년 10월 핵심 구성 요소 릴리스 2.2.0과 함께 도입된 v1입니다. 이 문서에서는 해당 구성 요소에 대해 설명합니다.
...
회전판, [탭,](tabs.md) 및 [아코디언 구성 요소](accordion.md) 구성 요소 내에서 패널로 직접 링크하기를 지원합니다.
...
패널 ID를 앵커로 사용하여 URL로 이동하면 브라우저는 특정 구성 요소로 바로 스크롤하고 지정 패널을 표시합니다. 패널이 기본적으로 표시되지 않도록 구성된 경우 자동으로 로 스크롤됩니다.
...
* **레이블** - 캐러셀의 콘텐츠를 설명하는 캐러셀의 아리아 레이블 속성 값
* **이전** - 캐러셀 탐색의 이전 버튼 레이블에 대한 아리아 레이블 특성의 값
* **다음** - 캐러셀 탐색의 다음 버튼 레이블에 대한 아리아 레이블 특성의 값
* **재생** - 캐러셀 탐색의 재생 버튼 레이블에 대한 아리아 레이블 특성의 값입니다.
* **일시 중지** - 캐러셀 탐색의 일시 중지 버튼 레이블에 대한 아리아 레이블 특성의 값입니다.
* **탭리스트** - 캐러셀 탐색의 항목 목록 레이블에 대한 아리아 레이블 특성의 값입니다.
* **항목의 아리아 레이블을 제목으로 설정** - 이 옵션을 선택하면 캐러셀 항목 제목이 아리아 레이블 설명으로 자동 설정됩니다.
...
* **제어 요소 사전 준비** - 이 옵션을 선택하면 접근성을 개선하기 위해 제어 요소가 캐러셀 항목 앞에 배치됩니다.
