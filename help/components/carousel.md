...
* **전환 지연** - 콘텐츠 작성자가 슬라이드 구성 요소를 페이지에 추가하면 슬라이드 간 전환 지연(밀리초)에 대한 기본값을 정의할 수 있습니다.
* **마우스 오버 시 자동 일시 중지 비활성화** - 콘텐츠 작성자가 **자동 전환 슬라이드**&#x200B;를 선택하여 자동 슬라이드 일시 중지를 비활성화하는 옵션이 활성화되면 정의합니다.
