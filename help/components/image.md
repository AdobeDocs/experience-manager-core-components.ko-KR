...
>[!TIP]
>
>기본적으로 이미지 구성 요소는 적응형 이미지 서블릿에 의해 제공됩니다. 다음을 참조하십시오 [적응형 이미지 서블릿](/help/developing/adaptive-image-servlet.md) 작동 방식에 대한 세부 정보.
...
* DAM에서 SVG 에셋 드래그 앤 드롭과 로컬 파일 시스템에서 업로드된 SVG 파일 업로드가 모두 지원됩니다.
...
이미지 구성 요소를 경험하고 구성 옵션의 샘플과 HTML 및 JSON 출력을 확인하려면 다음을 참조하십시오. [구성 요소 라이브러리](https://adobe.com/go/aem_cmp_library_image_kr).
...
다음을 보유하고 있는지 여부에 따라 [Dynamic Media](#dynamic-media) 활성화 또는 [차세대 Dynamic Media](#next-gen-dm) 기능을 사용할 수 있는 경우 이미지 편집에 사용할 수 있는 옵션이 다릅니다.
...
>[!NOTE]
>
>GIF 이미지의 경우 이미지 편집 작업이 지원되지 않습니다. 편집 모드에서 GIF에 대해 수행한 모든 변경 사항은 지속되지 않습니다.
...
* **이미지 자산** - **페이지에서 추천 이미지 상속**&#x200B;을 선택한 경우 자동으로 채워집니다. 다음 옵션을 설정하여 이미지를 수동으로 정의하려면 선택 해제합니다.
   ...
   * 에서 에셋 끌어 놓기 [에셋 브라우저](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/sites/authoring/fundamentals/environment-tools.html) 또는 을 누릅니다 **찾아보기** 로컬 파일 시스템에서 업로드할 수 있는 옵션입니다.
   * **지우기**&#x200B;를 탭하거나 클릭하여 현재 선택된 이미지 선택을 해제합니다.
   * 탭 또는 클릭 **선택** 을(를) 열려면 [에셋 브라우저](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/sites/authoring/fundamentals/environment-tools.html) 따라서 이미지를 선택할 수 있습니다.
...
* **대체 텍스트를 제공하지 않음** - 이미지가 단순히 장식용이거나 페이지에 추가 정보를 전달하지 않는 경우 화면 판독기와 같은 보조 기술에서 무시되도록 이미지를 표시합니다.
...
* **사전 설정 유형** - 이는 사용 가능한 이미지 사전 설정 유형, **이미지 사전 설정** 또는 **스마트 자르기** 중 하나를 정의하고, [Dynamic Media 기능](#dynamic-meida)이 활성화되는 경우에만 사용할 수 있습니다.
   ...
   * **스마트 자르기** - 다음의 경우 **사전 설정 유형** / **스마트 자르기** 이(가) 선택되어 있으면 드롭다운이 **렌디션** 를 사용할 수 있으며, 선택한 에셋에 대해 사용 가능한 렌디션을 선택할 수 있습니다. 선택한 자산에 대해 렌디션이 정의되는 경우에만 사용할 수 있습니다.
   * **이미지 수정자** - **사전 설정 유형** 선택에 관계없이 명령을 제공하는 추가 Dynamic Media 이미지를 `&`로 구분하여 정의할 수 있습니다.
* **캡션** - 기본적으로 이미지에 대한 추가 정보가 이미지 아래에 표시됩니다.
   * **DAM에서 캡션 가져오기** - 확인 표시가 되어 있으면 이미지의 캡션 텍스트가 `dc:title` DAM의 메타데이터.
   * **캡션을 팝업으로 표시** - 확인 시, 캡션은 이미지 아래에 표시되지 않고 마우스로 이미지를 가리키면 일부 브라우저에서 팝업으로 표시됩니다.
* **링크** - 이미지를 다른 리소스에 연결합니다.
   * 선택 대화 상자를 통해 다른 AEM 리소스에 연결합니다.
   * AEM 리소스에 연결되지 않은 경우 절대 URL을 입력합니다. 절대 URL이 아닌 URL은 AEM에 상대적인 것으로 해석됩니다.
   * **새 탭에서 링크 열기** 옵션을 통해 새 브라우저 창에서 링크를 열 수 있습니다.
* **ID** - 이 옵션을 통해 HTML 및 의 구성 요소 고유 식별자를 제어할 수 있습니다. [데이터 레이어](/help/developing/data-layer/overview.md).
...
>[!TIP]
>
>**스마트 자르기**&#x200B;와 **이미지 사전 설정**&#x200B;은 함께 수행할 수 없는 옵션입니다. 작성자가 스마트 자르기 렌디션과 함께 이미지 사전 설정을 사용해야 하는 경우 작성자는 **이미지 수정자** 을 클릭하여 사전 설정을 수동으로 추가합니다.
...
에서 이 구성 요소에 대한 스타일을 구성해야 합니다. [디자인 대화 상자](#design-dialog) 드롭다운 메뉴를 사용할 수 있습니다.
...
* **웹에 최적화된 이미지 활성화** - 확인 시, [웹에 최적화된 이미지 제공 서비스](/help/developing/web-optimized-image-delivery.md) 는 WebP 형식의 이미지를 제공하여 이미지 크기를 평균 25% 줄일 수 있습니다.
   * 이 옵션은 AEMaaCS에서만 사용할 수 있습니다.
   * 선택하지 않거나 웹 최적화된 이미지 제공 서비스를 사용할 수 없는 경우 [적응형 이미지 서블릿](/help/developing/adaptive-image-servlet.md) 를 사용합니다.
* **소극적 로드 비활성화** - 선택하면 구성 요소가 소극적 로드 없이 모든 이미지를 미리 로드합니다.
...
* **폭 조정** - 이 값은 DAM 자산인 베이스 이미지의 폭을 조정하는 데 사용됩니다.
   * 이미지의 종횡비는 그대로 유지됩니다.
   * 값이 이미지의 실제 폭보다 크면 이 값은 영향을 주지 않습니다.
...
* **폭** 이미지에 대해 폭 목록을 픽셀 단위로 정의하면 구성 요소는 브라우저 크기에 따라 가장 적절한 폭을 자동으로 로드합니다.
   ...
   * 기본적으로 이미지가 표시될 때까지 이미지 로드를 연기합니다.
      * 옵션 선택 **소극적 로드 비활성화** 따라서 페이지 로드 시 이미지를 로드할 수 있습니다.
* **JPEG 품질** - 변환된(예: 크기 조정된 또는 잘린) JPEG 이미지의 품질 계수(0~100 사이의 백분율)입니다.
