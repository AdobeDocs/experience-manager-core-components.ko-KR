...
Teaser 구성 요소의 현재 버전은 v2이며, 2022년 2월에 핵심 구성 요소 릴리스 2.18.0과 함께 도입되었으며 이 문서에 설명되어 있습니다.
...
![Teaser 구성 요소의 편집 대화 상자 링크 탭](/help/assets/teaser-edit-links.png)

티저 제목, 설명 및 이미지는 연결된 페이지 또는 첫 번째 콜 투 액션에서 연결된 페이지에서 상속될 수 있습니다. 링크나 콜 투 액션을 지정하지 않으면 현재 페이지에서 제목, 설명 및 이미지가 상속됩니다.

* **링크** - 이 파일은 컨텐츠 페이지, 외부 URL 또는 페이지 앵커에 연결됩니다.
* **새 탭에서 링크 열기** - 활성화된 경우 링크가 새 브라우저 탭에서 열립니다.
* **클릭유도문안** - 이 옵션을 사용하면 여러 대상에 연결할 수 있습니다.
   * 첫 번째 클릭유도문안에 연결된 페이지는 티저 제목, 설명 또는 이미지를 상속할 때 사용됩니다.
...
* **페이지에서 주요 이미지 상속** - 연결된 페이지의 페이지 속성에 정의된 이미지를 사용하거나 현재 페이지가 없는 경우 해당 이미지를 사용합니다.
* **이미지 자산** - 자산에서 삭제 [자산 브라우저](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/sites/authoring/fundamentals/environment-tools.html) 또는 **찾아보기** 로컬 파일 시스템에서 업로드할 옵션입니다.
...
* **접근성을 위한 대체 텍스트** - 이 필드에서는 시각 장애가 있는 사용자를 위해 이미지에 대한 설명을 정의할 수 있습니다.
   * **페이지에서 대체 텍스트 상속** - 이 옵션은 `dc:description` 자산이 연결되어 있지 않으면 DAM 또는 현재 페이지의 메타데이터입니다.
* **대체 텍스트를 제공하지 않음** - 이 옵션은 이미지가 단순한 장식용이거나 추가 정보가 페이지에 전달되지 않는 경우 화면 판독기와 같은 보조 기술에 의해 무시되도록 이미지를 표시합니다.
...
![티저 목록 구성 요소의 편집 대화 상자에 있는 스타일 탭](/help/assets/teaser-edit-styles.png)

Teaser 구성 요소는 AEM을 지원합니다 [스타일 시스템.](/help/get-started/authoring.md#component-styling).
...
* **기본 제목 유형** - 티저의 제목에 사용할 H 태그를 정의합니다.
