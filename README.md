---
source-git-commit: 78c755d553fdb424fee36bcb2a2c52627c51eaad
workflow-type: tm+mt
source-wordcount: '156'
ht-degree: 4%

---
# Adobe Experience Manager 핵심 구성 요소 설명서

## 핵심 구성 요소 설명서

Adobe Experience Manager 핵심 구성 요소 ***설명서***&#x200B;의 저장소입니다. 이 설명서에서는 설치, 제작 용도 및 핵심 구성 요소를 사용자 정의하는 방법에 대해 설명합니다.

설명서에 대한 귀하의 기여를 환영합니다. 다음 리소스를 참조하십시오.

* [contributing.md](contributing.md) 문서에 기여하는 방법에 대한 개요입니다.
* [guidelines.md](guidelines.md) 기여에서 기대하는 내용과 설명서 기여 작성 방법에 대한 개요입니다.
* [code-of-conduct.md](code-of-conduct.md) 이 설명서 프로젝트에 기여할 때 저희 팀에서 기대하는 활동 표준에 대한 개요입니다.

## 핵심 구성 요소 저장소

[Adobe Experience Manager 핵심 구성 요소](https://github.com/adobe/aem-core-wcm-components) 자체는 개별 저장소의 프로젝트로 유지됩니다. 구성 요소에 대한 기술/개발자 설명서가 필요하고, 구성 요소 자체에 영향을 미치거나 핵심 구성 요소의 기능을 사용하여 문제를 제기해야 하는 경우 해당 프로젝트를 확인하십시오.

